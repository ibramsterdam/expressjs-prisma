...
export async function updateRoleOfUserOnDatapodService(
  ...
  try {
    const userThatUpdates = await prisma.userOnDatapodwithRole.findUnique({
      where: {
        userId_datapodId: { userId: jtwPayload.sub, datapodId },
      },
    });
    ...
    if (
      userThatUpdates.role_name !== "Manager" &&
      userThatUpdates.role_name !== "Owner"
    ) {
      return { error: "You don't have permission to update a role" };
    }
    ...
    if (userThatUpdates.role_name === "Manager" && role_name === "Owner") {
      return {
        error: "Can't update someone to Owner status as a Manager yourself",
      };
    }

    console.log(
      "For logs in the console; This is expiration date passed:",
      expiration_date
    );
    ...
    console.log("Exp date after the check: ", newExpirationDate);
    ...
    const updatedUser = await prisma.userOnDatapodwithRole.update({
      ...
      data: {
        role_name: role_name,
        roleExpireDate: role_name === "Owner" ? null : newExpirationDate,
      },
    });

    if (!updatedUser) return { error: "Can't update a user for some reason" };
